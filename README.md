...
| 축 | 무엇 | 어떻게 |
|---|---|---|
| ① 공유 범위 | 나만 vs 사내 동료도 | server.py가 이미 `0.0.0.0` 바인딩 → `localhost:8000`은 나만, `<서버IP>:8000`은 같은 망 누구나. **별도 설정 없음** |
...
- **사내 서버 + OM까지 원하면** → omc+HelmholtzMedia 깔린 PC(보통 본인 작업 PC)에서 `./run.sh` → 동료는 `http://<그 PC IP>:8000`. 그러면 동료 브라우저에서 누른 Modelica 계산도 **그 서버의 omc**가 돌려서 결과만 전달.
