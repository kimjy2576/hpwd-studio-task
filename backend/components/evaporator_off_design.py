...
modelDescription = {
    ...
    'variables': [
        ...
        # Fitting — UA 모드 (둘 다 정의해두고 input_mode로 선택)
        {'name': 'UA_2ph', 'causality': 'parameter', 'type': 'Real',
         'group': 'Fitting', 'start': 200.0, 'unit': 'W/K',
         'description': '2-phase 영역 UA (input_mode=UA 시 사용)'},
        {'name': 'UA_SH', 'causality': 'parameter', 'type': 'Real',
         'group': 'Fitting', 'start': 30.0, 'unit': 'W/K',
         'description': 'Superheat 영역 UA (input_mode=UA 시 사용)'},
...
def step(input, params, state, dt):
    ...
    UA_2ph      = float(params.get('UA_2ph', 200.0))
    UA_SH       = float(params.get('UA_SH', 30.0))
