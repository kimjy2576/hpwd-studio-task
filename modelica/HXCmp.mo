package HXCmp "HX 단품 충실도 비교 — 동일 냉매입구·공기조건서 L1/L2/L3 Cond·Evap 대조"
  // [통일 BC — L3 사이클 정착점 기준, 공기 오픈루프 2.42 CMM]
  //  응축기: m=0.00227074 kg/s, P_in=9.912bar, h_in=627.818 kJ/kg, 공기 14.135°C/RH0.993
  //  증발기: m=0.00227074 kg/s, P_in=5.000bar, h_in=365.313 kJ/kg(2상), 공기 20°C/RH0.8
  //  L3 기준출력: Cond h_out=364.422 kJ/kg Q=598.80 W / Evap h_out=587.269 kJ/kg Q=503.89 W
  // 방법 격리: L1 ε-NTU 평균ΔT / L2 MB 이동경계+Shah / L3 FV 셀-march 입구ΔT.

  // ───────────────────── 응축기 ─────────────────────
  model Cond_L1 "L1 UA 집중 — Cond_UA_eq"
    HPWDhx.FlowSource src(p = 9.912e5, h = 627.818e3, m_flow_set = 0.00227074);
  ...
  model Cond_L2m "L2 MB — UA 매칭(L_tube 10→7.8, 공기 19.1CMM)해 L1/L3와 conductance 근사"
    HPWDhx.FlowSource src(p = 9.912e5, h = 627.818e3, m_flow_set = 0.00227074);
    CondMBe.CondenserSS cond(T_air_in_C = 14.135, RH_in = 0.993, L_tube_total = 7.8, V_air_CMM = 2.42);
    HPWDhx.SinkOpen snk;
  equation
    connect(src.port, cond.port_a);
    connect(cond.port_b, snk.port);
  ...
  model Cond_L3 "L3 유한체적 — Cond_On (셀-march)"
    HPWDevap.FlowSource src(m_dot = 0.00227074, h = 627.818e3, p = 9.912e5);
  ...
  model Evap_L1 "L1 UA 집중 — Evap_UA_eq (UA를 L3 유효UA≈80에 매칭: 2ph=64,SH=16)"
    HPWDhx.FlowSource src(p = 5.000e5, h = 365.313e3, m_flow_set = 0.00227074);
    HPWDhx.Evap_UA_eq evap(T_air_in = 20.0 + 273.15, RH_in = 0.8, V_air_CMM = 2.42, UA_2ph = 64.0, UA_SH = 16.0);
  ...
  model Evap_L2 "L2 MB 동적 — EvaporatorMBdyn (FlowBC P_e 자기조절, 5g/s 안정점)"
    ...
    EvapMBe.FlowBC inlet(m_flow_set = -0.00227074, k_p = 0.0, h_set = 365.313e3);
    EvapMBe.FlowBC outlet(m_flow_set = 0.0, k_p = 0.00227074/5.0e5, h_set = 0.0);
  equation
  ...
  model Evap_L3 "L3 유한체적 동적 — Evap_On (셀-march)"
    HPWDevap.FlowSource src(m_dot = 0.00227074, h = 365.313e3, p = 5.000e5);
