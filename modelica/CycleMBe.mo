...
package CycleMBe "방정식형 MB 사이클: EvaporatorSS + Comp_Winandy + CondenserSS + EEV → 폐루프 (정상상태 대수해)"
  ...
  function propsEvap "P_e[Pa] → 포화·과열 스칼라 물성 (HelmholtzMedia record는 함수 내부 격리)"
    input Real p "[Pa]";
  ...
  algorithm
    sat := M.setSat_p(p);
    Tevap := M.saturationTemperature(p);
