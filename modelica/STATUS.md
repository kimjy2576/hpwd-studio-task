...
## Pe 타겟팅 (evap UA·N, 2026-05-23)
레퍼런스 Pe=5.5bar(Tsat 4.9°C, 차가운 코일 → 강한 제습) 목표. air 3.0/3.0 고정, SH=6 PI 유지.

### 레버 방향 (확인됨)
- **evap UA↓** → ε↓ → 같은 흡열에 ΔT↑ 필요 → T_evap↓ → **Pe↓**. *부수*: ṁ↓·Pc↓·W↓ (사이클 용량 동반 축소).
- **N↑** → ṁ↑ → 증발기 더 당김 → Pe↓. 단 Pc도 ↑ (25bar 한계 제약).
- evap UA가 주 레버. evap UA scale j 스윕(N3000): j1.0→Pe8.44, j0.5→6.48, j0.37→5.66, **j0.35→5.51**, j0.3→5.11.

### ★ 채택: evap UA_2ph=8.75 / UA_SH=1.4 (원 25/4의 ×0.35), N=3000
정상상태: **Pc 21.0 / Pe 5.51 bar**, ṁ 4.83 g/s, SH 6.00K, EEV 69.5%, W 487W, Q_evap≈853W, **COP≈1.75**, charge 120g.
- Pe 타겟 5.5 적중. **부수효과로 ṁ 7.33→4.83 ↓ → 응축부하↓ → Pc 24→21 동반 하락**(타겟 19에 더 근접).
  evap UA 한 레버로 Pe·Pc 둘 다 개선. 19/5.5 레퍼런스에 근접한 첫 운전점(잔차: Pc +2bar = air-side 천장).
- ṁ 4.83 g/s는 **원 컴포넌트 단독검증 설계유량(~4.6-5)과 일치** → 자기일관 설계점.
...
2. **공기루프 동특성 결합** (팬·필터·덕트·드럼) — 현재 air 입구 시불변 고정. 결합되면 evap 입구 T·RH가
   건조진행에 따라 변동 → 운전점 재정렬 필요. decoupled 가정의 마지막 단계.
3. PI 게인 튜닝(t≈20 언더슈트 4.5K 완화), L2 전환(Winandy/Sami-Schnotale/MovingBoundary).
