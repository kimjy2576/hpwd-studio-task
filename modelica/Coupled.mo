package HPWDcpl "냉매-공기 커플드 HX (L1) — RefPort + AirPort 양쪽 포트"
  ...
  // =============================================================
  // ══ 3급 커플드 공통 조건 (링 공정비교서 확립한 교란요인 제거 반영) ══
  //  · 필터 삽입(drum 직후) + volF — 실제 건조기 구조, 3급 토폴로지 통일
  //  · 필터 fit: L3 메쉬(MPI15/d_w0.4/L0.6) 기준. L1 K=10.2698, L2 a=4.9186e5·b=5.5091
  //    ⚠️ fit은 링 운전점(u~2.3) 기준. 커플드는 저유량(u~0.7)이라 L1 필터 근사가
  //       다소 벗어남(dp 차 ~3Pa, 전체 200Pa 대비 ~1.5%). 영향 작아 그대로 사용.
  //  · 드럼 BC 통일: Tcl0=305.0, UA_amb=0 (Drum_L3엔 UA_amb 파라미터 없음)
  //  · 팬 기하 전 통일: D2=0.15, b2=0.04, D1=0.120, b1=0.060, Z=40, β2=150, β1=30
  //  · 팬 N=2200: N=3000이면 공기부하가 증발기 용량 10% 초과 → SH 폭주(t=0.076서
  //    Integrator failed). 부하비 1.10 → RPM↓로 해소. PI가 SH=6K 정착 확인.
  model Cycle_coupled_closed
    "커플 HPWD L1 공기측: 냉매 사이클 + 공기 폐루프(Drum_L1 항률 + Filter_L1 + Fan_L1 일괄효율)"
    // ── 냉매 사이클 (3급 공통, Cycle_coupled_open과 동일 init) ──
    HPWD.Comp_Theoretical comp(t_ramp = 0.0);
    HPWDcycle.Volume vol1(p_start = 16e5,  h_start = 665e3, fixedState = true);
    ...
    HPWDair.Filter_L1 filt(A_face = 0.05, r_pleat = 1.0, theta_face = 0, K = 10.2698);
    HPWDair.Fan_L1 fan(
      D2 = 0.15, b2 = 0.04, Z = 40, beta2 = 150,
      eta_h = 0.78, eta_mech = 0.95, N = 2200);
    HPWDair.AirVolumeC volRef(
      V = 0.05, p_start = HPWDair.MoistAir.p_ref,
      T_start = 304.0, W_start = 0.018, fixedState = true);
    HPWDair.AirVolume volF(V = 0.05, T_start = 304.0, W_start = 0.018, fixedState = true);
    HPWDair.AirVolume volB(V = 0.05, T_start = 305.0, W_start = 0.018,
      fixedState = true, UA_amb = 100.0, T_amb = 298.15);
      // ★ 닫힌 커플드계의 열 배출구 (3급 공통, fidelity-중립).
      //   냉매루프+공기루프가 모두 닫혀 압축기·팬 일이 갈 곳이 없음
      //   → 없으면 계 무한 승온(p_evap 6.5→17.9bar, T_air 32→56°C @t1200).
      //   드럼 UA_amb 대신 볼륨에 둔 이유: Drum_L3엔 UA_amb 파라미터가
      //   없어 드럼에 두면 3급 열침이 불균등 → 비교가 오염됨.
  ...
  equation
    ...
    connect(evap.SH, ctrl.SH_meas);
    connect(ctrl.opening, eev.opening);
    // ── 공기 폐루프: drum→volRef→filt→volF→fan→volB→evap→volC→cond→volD→drum ──
  ...
  model Cycle_coupled_closed_L2air
    "커플 HPWD L2 공기측: Drum_L2(감률/흡착) + Filter_L2(DF 2항) + Fan_L2(손실분해+스크롤)"
    // ── 냉매 사이클 (3급 공통, Cycle_coupled_open과 동일 init) ──
    HPWD.Comp_Theoretical comp(t_ramp = 0.0);
    HPWDcycle.Volume vol1(p_start = 16e5,  h_start = 665e3, fixedState = true);
    HPWDcpl.Cond_coupled cond(m_dot(start = 0.006), K_air = 300);
    ...
    HPWDair.Drum_L2 drum(
      ...
      A_drum = 0.15, K_drum = 30, X0 = 0.6, Tcl0 = 305.0,
      UA_amb = 0.0, T_amb = 298.15);
    HPWDair.Filter_L2 filt(A_face = 0.05, r_pleat = 1.0, theta_face = 0,
      a_visc = 4.9186e5, b_inert = 5.5091);
    HPWDair.Fan_L2 fan(
      D2 = 0.15, b2 = 0.04, D1 = 0.120, b1 = 0.060, Z = 40,
      beta2 = 150, beta1 = 30, eta_mech = 0.95, N = 2200,
      k_scroll = 0.2842);
      // ⚠️ k_scroll 재교정: 기본 0.25는 D2=0.175/N=3000 기하서 교정한 값.
      //    커플드 기하(D2=0.15/N=2200)선 L3의 dP_scroll/(0.5ρc2²)가 0.2842 →
      //    기본값 쓰면 L2가 L3 대비 11% 과대. 계수 1개로 스크롤을 대표하는
      //    L2의 구조적 한계 — 기하 바뀌면 재교정 필요(L2 유효범위 경계).
    HPWDair.AirVolumeC volRef(
      V = 0.05, p_start = HPWDair.MoistAir.p_ref,
      T_start = 304.0, W_start = 0.018, fixedState = true);
    HPWDair.AirVolume volF(V = 0.05, T_start = 304.0, W_start = 0.018, fixedState = true);
  ...
  model Cycle_coupled_closed_L3air
    "커플 HPWD L3 공기측: Drum_L3(3-Zone 4경로) + Filter_L3(메쉬 Ergun) + Fan_L3(Meanline 9손실).
     drum_on/filter_on/fan_on 검증 L3 3컴포넌트 + 냉매 사이클 = 최고충실도 HPWD 전체 시스템."
    // ── 냉매 사이클 (3급 공통, Cycle_coupled_open과 동일 init) ──
    HPWD.Comp_Theoretical comp(t_ramp = 0.0);
    HPWDcycle.Volume vol1(p_start = 16e5,  h_start = 665e3, fixedState = true);
    HPWDcpl.Cond_coupled cond(m_dot(start = 0.006), K_air = 300);
    ...
    HPWDair.Drum_L3 drum(
      ...
      drum_radius = 0.27, drum_length = 0.45, RPM = 45.0, K_drum = 30);
    HPWDair.Filter_L3 filt(A_face = 0.05, r_pleat = 1.0, theta_face = 0);
    HPWDair.Fan_L3 fan(
      D2 = 0.15, b2 = 0.04, D1 = 0.120, b1 = 0.060, Z = 40,
      beta2 = 150, beta1 = 30, N = 2200);
    HPWDair.AirVolumeC volRef(
      V = 0.05, p_start = HPWDair.MoistAir.p_ref,
      T_start = 304.0, W_start = 0.018, fixedState = true);
    HPWDair.AirVolume volF(V = 0.05, T_start = 304.0, W_start = 0.018, fixedState = true);
  ...
  equation
    ...
    connect(cond.air_b,    volD.port_a);
    connect(volD.port_b,   drum.port_a);
  end Cycle_coupled_closed_L3air;
