package HPWDcpl "냉매-공기 커플드 HX (L1) — RefPort + AirPort 양쪽 포트"
  ...
  equation
    ...
    T_evap_K = Ref.saturationTemperature(max(P_evap, 1e5));
    ...
    T_ref_out_C = Ref.temperature(Ref.setState_ph(max(port_b.p, 1e5), h_out_ref)) - 273.15;
  ...
  equation
    ...
    T_cond_K = Ref.saturationTemperature(max(P_cond, 1e5));
  ...
  //
  // 현 상태(검증, K_air=500/UA_amb=100): 구조 완성·물/에너지 균형(WBAL~0.0001,
  //   EBAL~-0.5W)·Pc 안정 수렴·드럼 건조 진행. checkModel 249/249.
  //   운전점: Pc=29.7bar Pe=8.7bar, T_cond=77°C T_evap=22°C, W_comp=689W,
  //   SMER=1.73 kg/kWh (실 HPWD 범위), 건조 X 0.6->0.05 ~1.4hr.
  //   공기유량 0.042kg/s (K_air=500 throttle). 외기손실 Qamb~690W로 아직 큼 —
  //   더 식히려면(SMER↑) K_air↓(공기↑)+UA_amb↓ 가능하나 init 재튜닝 필요
  //   (운전점 init이 파라미터 급변에 취약: K_air<200 등 큰 변경시 초기천이 발산).
  // =============================================================
  model Cycle_coupled_closed
    ...
    HPWD.Comp_Theoretical comp(t_ramp = 0.0);
    HPWDcycle.Volume vol1(p_start = 16e5,  h_start = 665e3, fixedState = true);
    HPWDcpl.Cond_coupled cond(m_dot(start = 0.006), K_air = 500);
    HPWDcycle.Volume vol2(p_start = 15.5e5, h_start = 345e3, fixedState = true);
    HPWDcycle.EEV_Orifice_ctrl eev(m_dot(start = 0.006));
    HPWDcycle.Volume vol3(p_start = 6.5e5, h_start = 345e3, fixedState = true);
    HPWDcpl.Evap_coupled evap(m_dot(start = 0.006), K_air = 500);
    HPWDcycle.Volume vol4(p_start = 6.4e5, h_start = 595e3, fixedState = true);
    HPWDctrl.PI_Controller ctrl(I(fixed = true));
