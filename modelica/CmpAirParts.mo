package CmpAirParts "공기측 부품 단독 충실도 비교 — 동일 BC에서 L1/L2/L3 팬·드럼·필터 대조"
  ...
  // ── 공통 팬 기하 (3급 동일 = 같은 물리 팬) ──
  //   Fan_L3 기본값 기준 (fan-sim 검증 레퍼런스). 이전엔 N만 통일하고
  //   기하는 각자 기본값(L1 D2=0.15/Z=40/β2=150 vs L3 D2=0.175/Z=36/β2=145)
  //   → 서로 다른 팬을 비교하던 오류. 통일함.
  //   ⚠️ 실제 HPWD 팬 기하 미확정 (fan-sim 대표값, 팬/드럼/필터 공통 상황).
  constant Real D2_fan = 0.175 "임펠러 외경 (m)";
  constant Real b2_fan = 0.050 "출구 블레이드 폭 (m)";
  constant Real D1_fan = 0.120 "임펠러 eye 직경 (m)";
  constant Real b1_fan = 0.060 "입구 블레이드 폭 (m)";
  constant Integer Z_fan = 36 "블레이드 수";
  constant Real beta2_fan = 145 "블레이드 출구각 (deg, >90=전곡)";
  constant Real beta1_fan = 30 "블레이드 입구각 (deg)";
  ...
  model Fan_L1 "L1 팬 — Euler+Stodola slip (η_h 상수, 전곡)"
    ...
    HPWDair.Fan_L1 fan(N = N_fan, D2 = D2_fan, b2 = b2_fan,
      Z = Z_fan, beta2 = beta2_fan);
    HPWDair.BoundaryAir_mflow outlet(m_flow_da = m_fan, T = T_fan, W = W_fan);
    // ⚠️ 부호: 팬/필터는 outlet이 BoundaryAir_mflow → +m (정방향).
    //    -m이면 역류(Fan_L3 eta=0). 드럼은 inlet이 mflow라 -m이 정방향.
  equation
  ...
  model Fan_L2 "L2 팬 — +incidence·skin-friction 손실"
    HPWDair.BoundaryAir_pTW inlet(p = 101325.0, T = T_fan, W = W_fan);
    HPWDair.Fan_L2 fan(N = N_fan, D2 = D2_fan, b2 = b2_fan,
      D1 = D1_fan, b1 = b1_fan, Z = Z_fan,
      beta2 = beta2_fan, beta1 = beta1_fan);
    HPWDair.BoundaryAir_mflow outlet(m_flow_da = m_fan, T = T_fan, W = W_fan);
    // ⚠️ 부호: 팬/필터는 outlet이 BoundaryAir_mflow → +m (정방향).
    //    -m이면 역류(Fan_L3 eta=0). 드럼은 inlet이 mflow라 -m이 정방향.
  equation
  ...
  model Fan_L3 "L3 팬 — Meanline 9손실 (fan_on.py 검증)"
    HPWDair.BoundaryAir_pTW inlet(p = 101325.0, T = T_fan, W = W_fan);
    HPWDair.Fan_L3 fan(N = N_fan, D2 = D2_fan, b2 = b2_fan,
      D1 = D1_fan, b1 = b1_fan, Z = Z_fan,
      beta2 = beta2_fan, beta1 = beta1_fan);
    HPWDair.BoundaryAir_mflow outlet(m_flow_da = m_fan, T = T_fan, W = W_fan);
    // ⚠️ 부호: 팬/필터는 outlet이 BoundaryAir_mflow → +m (정방향).
