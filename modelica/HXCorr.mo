package HXCorr "HX Moving-Boundary correlation 함수 라이브러리 (Python 원본 충실 포팅)"
  ...
  // ════════════ Micro-fin 내부강화 (Carnavos 단상 / Cavallini-Diani 2상) ════════════
  function microfin_area_ratio "내부면적 강화비 ψ (핀 플랭크 ~2·n·e, 나선 1/cosβ). geometry.py와 동일."
    input Integer n_microfin "내부 핀 개수";
    input Real e_microfin "핀 높이 [m]";
    input Real helix_angle "나선각 [deg]";
    input Modelica.Units.SI.Diameter Di;
    output Real psi;
  protected
    Real beta_, fin_perim;
  algorithm
    if n_microfin > 0 and e_microfin > 0.0 then
      beta_ := helix_angle*3.141592653589793/180.0;
      fin_perim := 2.0*n_microfin*e_microfin/max(cos(beta_), 0.1);
      psi := 1.0 + fin_perim/(3.141592653589793*Di);
    else
      psi := 1.0;
    end if;
  end microfin_area_ratio;

  function microfin_ef
    "micro-fin 강화계수 EF = h_microfin/h_smooth (nominal Di 기준). correlations.py microfin_ef와 동일.
     매끈관 HTC에 곱. regime: 'single'(단상 Carnavos: sec³β·ψ^0.5) /
     'cond'(응축 Cavallini-Diani: secβ·ψ^0.8) / 'evap'(증발: secβ·ψ^0.75). cf_hi로 최종보정."
    input String regime;
    input Real area_ratio "ψ (microfin_area_ratio)";
    input Real helix_angle "[deg]";
    output Real ef;
  protected
    Real psi, beta_, sec_;
  algorithm
    ...
    beta_ := helix_angle*3.141592653589793/180.0;
    sec_ := 1.0/max(cos(beta_), 0.1);
    if regime == "single" then
      ef := sec_^3*psi^0.5;
    elseif regime == "cond" then
      ef := sec_*psi^0.8;
    elseif regime == "evap" then
      ef := sec_*psi^0.75;
    else
      ef := 1.0;
    end if;
  ...
  function inline_arr_ratio_j
    "in-line/staggered 공기측 j 비 (finned, fin-dominated). Grimison/Zukauskas arrangement, Pt/Pl 의존.
     correlations.py _inline_arr_ratio_j와 동일."
    input Real Pt;
    input Real Pl;
    output Real r;
  protected
  ...
  algorithm
    PtPl := Pt/max(Pl, 1e-9);
    base_ := 0.90;
    r := base_ + 0.05*min(max(PtPl - 1.0, 0.0)/0.5, 1.0);
    r := min(max(r, 0.88), 0.96);
  end inline_arr_ratio_j;

  function j_plain_inline
    "Plain fin IN-LINE 배열 j = Wang(2000) staggered × 튜브뱅크 arrangement 비. correlations.py j_plain_inline와 동일.
     standalone in-line plate-fin 상관식 부재 → 검증 staggered + arrangement 결합. (Modelica는 air-f 미사용→j만)"
    input Real Re_Dc;
    input Integer Nr;
    input Real Dc, Pt, Pl, FPI, fin_thickness;
    output Real j;
  algorithm
    j := max(j_wang2000_plain(Re_Dc, Nr, Dc, Pt, Pl, FPI, fin_thickness)*inline_arr_ratio_j(Pt, Pl), 1e-6);
  end j_plain_inline;
