...
package HPWDevap "L3 증발기 2D 컬럼 (Nr×N_seg, 동적 습/건, 공기 행진행)"
  ...
  model Evap_On "L3 증발기 — RefPort 음함수 컴포넌트 (1컬럼×Nt, 공기 경계)"
    ...
    parameter Real T_air_in=20.0 "공기 입구온도 [degC] (드럼 출구 공기)";
    parameter Real RH_in=0.8;
    parameter Real m_air_seg=0.00119464 "(col,seg)당 공기유량 [kg/s]";
    // ── 공기측 h_o — 하드코딩 제거, 형상·유동서 산출 (증발기: FPI=20) ──
    parameter Real P_t=14.14e-3, P_l=10e-3 "튜브 피치 [m]";
    parameter Real FPI=20.0 "핀 밀도 [fins/inch]";
    parameter Real A_o_face=0.0135744 "공기측 전면적 [m2]";
    parameter Real V_air_CMM_ho=2.42 "h_o 산출용 공기유량 [CMM] (L3 BC와 일치)";
    ...
    final parameter Real m_air_ho=HXCorr.rho_humid_air(T_air_in, W_ho, Patm)*(V_air_CMM_ho/60.0);
    ...
    parameter Real Di=0.0046;
    parameter Real A_i_seg=0.0003468318, A_o_seg=0.0048955023;
    parameter Real Dc=0.005220, Xm=0.007070, XL=0.00612342, k_fin=200.0, fin_t=0.11e-3;
    parameter Real A_fin_ratio=0.9265582679;
    parameter Real Patm=101325.0, Pcrit=4.2512e6, M_mol=44.0956;
  ...
  model Cond_On "L3 응축기 — RefPort 음함수 컴포넌트 (dry, Shah 응축, 공기 가열)"
    ...
    parameter Real m_air_seg=0.00119464 "(col,seg)당 공기유량 [kg/s] — 증발기와 동일(직렬 건공기 보존, 2.42 CMM)";
    ...
    parameter Real Di=0.0046;
    parameter Real A_i_seg=0.0003468318, A_o_seg=0.0053482610;
    parameter Real Dc=0.005220, Xm=0.007070, XL=0.00612342, k_fin=200.0, fin_t=0.11e-3;
    parameter Real A_fin_ratio=0.9333809461;
    parameter Real Patm=101325.0, Pcrit=4.2512e6, M_mol=44.0956;
    parameter Real A_cs=Modelica.Constants.pi*Di^2/4.0;
  ...
  model Cond_On_Dyn "L3 응축기 — 동적 유한체적 (냉매 엔탈피 h_ref + 벽온도 T_w 상태). 폐루프/콜드스타트용."
    ...
    parameter Real m_air_total=0.0477856 "코일 전체 공기유량 [kg/s] — 증발기와 동일(직렬 건공기 보존, 2.42 CMM)
      기존 m_air_seg=0.00119464 × Nt·Nseg(=40)와 정확히 동일 — 값 보존 리팩터.
      ※ V_air_CMM_ho(h_o 산출용)와 0.92% 불일치 (rho 1.1848 vs 1.1957). 별건 보류.";
    ...
    parameter Real H_coil=0.05656 "코일 높이 [m]";
    parameter Real D_coil=0.06 "코일 깊이 [m] (= Nr·P_l, 응축기 6행)";
    ...
    final parameter Real m_air_ho=HXCorr.rho_humid_air(T_air_in_start, W_ho, Patm)*(V_air_CMM_ho/60.0);
    final parameter Real G_air_ho=m_air_ho/A_c_ho "공기 질량속도 [kg/m2s]";
  ...
  model Evap_On_Dyn "L3 증발기 — 동적 유한체적 (냉매 엔탈피 h_ref + 벽온도 T_w 상태), 습코일. 폐루프/콜드스타트용."
    ...
    parameter Real T_air_in=20.0 "공기 입구온도 [degC] (드럼 출구 공기)";
    parameter Real RH_in=0.8;
    parameter Real m_air_total=0.0477856 "코일 전체 공기유량 [kg/s]
      기존 m_air_seg=0.00119464 × Nt·Nseg(=40)와 정확히 동일 — 값 보존 리팩터.
      세그당 값을 총량에서 산출하므로 Nseg/Nt를 바꿔도 총 풍량이 유지됨.
      ※ V_air_CMM_ho(h_o 산출용)와 0.92% 불일치 — 이 값은 rho=1.1848 상당,
        h_o 체인은 HXCorr.rho_humid_air=1.1957. 통일은 거동 변경이라 별건 보류.";
    final parameter Real m_air_seg=HXGeom.m_air_seg(m_air_total, Nt, Nseg) "(col,seg)당 공기유량 [kg/s]";
    // ── 1차 형상 (임의 구성 비교의 입력; 파생량은 HXGeom이 산출) ──
    parameter Real W_coil=0.24 "튜브 길이 = 코일 폭 [m]";
    parameter Real H_coil=0.05656 "코일 높이 [m]";
    parameter Real D_coil=0.04 "코일 깊이 [m] (= Nr·P_l)";
    parameter Real Do=0.005 "튜브 외경 [m]";
    // ── 공기측 h_o — 하드코딩 제거, 형상·유동서 산출 (증발기: FPI=20) ──
    parameter Real P_t=14.14e-3, P_l=10e-3 "튜브 피치 [m]";
    parameter Real FPI=20.0 "핀 밀도 [fins/inch]";
