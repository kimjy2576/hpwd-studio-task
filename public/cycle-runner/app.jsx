...
// ══════════════════════════════════════════════════════════════
// 커플드 사이클 다이어그램 (Coupled.mo 위상)
//   냉매 루프: comp → cond → eev → evap → comp (상단 사각 순환)
//   공기 루프: drum → filter → fan → evap → cond → drum (하단)
//   evap/cond = 두 루프 교차 컴포넌트 (한 몸, fidelity 하나)
// ══════════════════════════════════════════════════════════════
function CoupledCycle({ fid, setFid, fanPosition, setFanPosition, running, onEditParams }) {
  // 컴포넌트 노드 (fidelity 선택 + ⚙)
  const Node = ({ ck, label, en, color, icon, wide }) => (
    <div className={`node bg-white rounded-xl border-2 p-2.5 ${wide ? 'min-w-[140px]' : 'min-w-[112px]'}`}
      ...
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mono text-[9px] text-slate-400">{en}</div>
        </div>
        <button onClick={() => onEditParams(ck)} title="설계변수"
          className="text-sm px-0.5" style={{ color }}>⚙</button>
      </div>
      <FidelitySelector value={fid[ck]} onChange={v => setFid(ck, v)} />
    </div>
  );

  // 공유 컴포넌트(evap/cond) — 냉매+공기 교차 표시
  const SharedNode = ({ ck, label, en, color, icon }) => (
    <div className="node bg-gradient-to-br from-white to-slate-50 rounded-xl border-2 border-dashed p-2.5 min-w-[128px] relative"
      style={{ borderColor: color }}>
      <div className="absolute -top-2 left-1/2 -translate-x-1/2 px-1.5 py-0.5 bg-white rounded-full border text-[8px] mono font-semibold"
        style={{ borderColor: color, color }}>냉매+공기</div>
  ...
  const Arrow = ({ color, dir }) => (
    <div className="flex items-center justify-center shrink-0" style={{ color }}>
      <span className="text-lg">{dir === 'down' ? '↓' : dir === 'up' ? '↑' : dir === 'left' ? '←' : '→'}</span>
    </div>
  );
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-6">
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-sm font-bold text-slate-700">커플드 사이클</h3>
        <div className="flex items-center gap-3">
          <span className="mono text-[10px] flex items-center gap-1"><span className="w-2.5 h-0.5 inline-block" style={{background:'var(--cond)'}}></span>냉매</span>
          <span className="mono text-[10px] flex items-center gap-1"><span className="w-2.5 h-0.5 inline-block" style={{background:'var(--accent)'}}></span>공기</span>
        </div>
      </div>
      <p className="mono text-[10px] text-slate-400 mb-4">증발기·응축기는 냉매/공기 공유 (한 몸)</p>

      {/* 냉매 루프 (상단) — comp → cond → eev → evap → comp 순환 */}
      <div className="mb-2">
        <div className="mono text-[10px] font-semibold uppercase tracking-wide mb-2" style={{color:'var(--cond)'}}>냉매 루프 (폐회로)</div>
        <div className="flex items-center justify-center gap-2 flex-wrap">
          <Node ck="compressor" label="압축기" en="Compressor" color="var(--comp)" icon="⊙" />
          <Arrow color="var(--cond)" dir="right" />
          <SharedNode ck="condenser" label="응축기" en="Condenser" color="var(--cond)" icon="▤" />
          <Arrow color="var(--cond)" dir="right" />
          <Node ck="eev" label="팽창밸브" en="EEV" color="var(--eev)" icon="◇" />
          <Arrow color="var(--cond)" dir="right" />
          <SharedNode ck="evaporator" label="증발기" en="Evaporator" color="var(--evap)" icon="▤" />
          <div className="mono text-[10px] shrink-0" style={{color:'var(--cond)'}}>↺ comp</div>
        </div>
      </div>

      {/* 교차 표시 (냉매 evap/cond ↕ 공기 evap/cond) */}
...
function CycleRunner() {
  // 통합 fidelity — evap/cond는 냉매·공기 공유(한 몸), 하나만 지정
  //   refOnly: compressor, eev (냉매 전용)
  //   airOnly: drum, filter, fan (공기 전용)
  //   shared:  evaporator, condenser (냉매+공기 교차 컴포넌트)
  const [fid, setFid] = useState({
    compressor: 3, eev: 3,           // 냉매 전용
    drum: 1, filter: 1, fan: 3,      // 공기 전용
    evaporator: 3, condenser: 3,     // 공유 (교차)
  });
  ...
  const setFidKey = (k, v) => setFid({ ...fid, [k]: v });

  // 컴포넌트 fidelity 조회 (통합)
  const compFidelity = (comp) => fid[comp] || 1;
  const COMP_LABELS = { compressor: '압축기', condenser: '응축기', eev: '팽창밸브', evaporator: '증발기', fan: '팬', drum: '드럼', filter: '필터' };

  // 냉매 fidelity 표시용 (comp/cond/eev/evap)
  const refFidStr = ['compressor', 'condenser', 'eev', 'evaporator'].map(k => `L${fid[k]}`).join('·');

  // 파라미터 값 조회/변경 (comp+fidelity별)
  const getParamVals = (comp, f) => (paramValues[comp] && paramValues[comp][f]) || {};
  const setParamVal = (comp, f, name, value) => {
    setParamValues(prev => ({
      ...prev,
      [comp]: { ...(prev[comp] || {}), [f]: { ...((prev[comp] || {})[f] || {}), [name]: value } },
    }));
  };
  ...
  const runCycle = () => {
    ...
    // evap/cond는 공유 fidelity → ref/air 양쪽에 동일값 전달 (한 몸)
    const payload = {
      ref_fidelity: {
        compressor: fid.compressor, condenser: fid.condenser,
        eev: fid.eev, evaporator: fid.evaporator,
      },
      air_fidelity: {
        drum: fid.drum, filter: fid.filter, fan: fid.fan,
        evaporator: fid.evaporator, condenser: fid.condenser,  // 공유값 동일
      },
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-2 flex items-center gap-4">
        ...
        <div className="text-[11px] text-slate-500">Cycle Runner · 컴포넌트별 fidelity 자유 조합</div>
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-2 flex items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-slate-400 font-mono">HPWD R290</span>
        </div>
        <div className="flex-1"></div>
        <button
          onClick={runCycle}
      ...
      <main className="max-w-6xl mx-auto px-6 py-6 space-y-5">
        {/* 커플드 사이클 (냉매+공기 통합, evap/cond 공유) */}
        <CoupledCycle
          fid={fid} setFid={setFidKey}
