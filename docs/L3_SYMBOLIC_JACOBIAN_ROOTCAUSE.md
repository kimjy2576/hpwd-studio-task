...
## 4f. P5 판정 — 범용성 모드 성립 (2026-08-02, feat/analytic-medium)

v1(최대 일관 구성)의 symbolic 빌드는 1회차 통과했고(ASUB 0), 실행 판정은
컨테이너 스냅숏 제약으로 0~300 s(기동 과도·EEV 급조임·SH 형성·과거 정체
구간을 모두 포함하는 난코스)로 수행했습니다.

| 0~300 s 동일 창 | 완주 | 드리프트 | 비고 |
|---|---|---|---|
| v1 + coloredNumerical | O | −0.002 ~ **+0.690 %** | 과도 이탈 |
| **v1 + coloredSymbolical** | **O (21분)** | **−0.032 ~ +0.037 %** | **19배 개선, 기준 밴드 동급** |
| 기준(테이블+수치) | O | −0.071 ~ +0.013 % | 확정 경로 |

추가 실측: 정확 야코비안이 뉴턴을 살려 2 s 창에서 스텝 82→54, 야코비안
평가 70→31회로 반감. 회당 비용은 1.32 s(수치 73 ms 의 ~18배).

**결론 — v1 의 +0.69 % "혼용 드리프트"는 대부분 수치 야코비안의 적분
아티팩트였습니다.** 범용성 모드(해석+기호)는 속도 ~10배를 대가로
(1) 수렴 강건성(스텝 반감)과 (2) 보존 정확도(드리프트 19배)를 얻습니다.
이원화 전략(§4e)의 역할 분담이 실측으로 확정되었습니다.

미완: 300→600 s 완주는 워크스테이션 위임(브랜치 ba685c7, 실행 명령에
`-jacobian=coloredSymbolical` 만 추가). 체크포인트 재개(-iif/-iit)는
이산 상태(시퀀서·EEV 적분기) 미복원으로 기각 — 재개 인프라는 범용 툴
과제로 등재.
