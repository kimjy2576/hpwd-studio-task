...
## 발견 2 — Python vendor 습/건 이중안정성 → 연속 가중으로 해소 (2026-07-24)

### 원인 — 진동이 아니라 히스테리시스 이중안정성

vendor에는 이미 `a243da2`의 대책(±0.15 K dead-band + 적응 under-relax + flip 카운팅)이
들어 있었다. 그럼에도 경로순 습/건이 뒤집히는 이유를 셀 단위로 측정한 결과:

| k | T_w | T_w−T_dp | wet | 수렴 |
|---|---|---|---|---|
| 21~23 | 16.81~16.95 | +0.36~+0.50 | 건 | ✓ |
| 24~26 | 16.33~16.41 | −0.04~−0.11 | **습** | ✓ |
| 27~30 | 16.37~16.39 | −0.06~−0.08 | **건** | ✓ |

**T_w가 사실상 같은 셀들(16.33~16.41, 모두 노점 아래)이 습/건 반대 판정.**
각 셀은 독립 반복이고 초기추정 `T_w=(T_air+T_ref)/2`에서 출발하는데,
초기 상태가 습이면 습으로, 건이면 dead-band 0.15 K를 못 넘어 건으로 굳는다.
미수렴은 2/40뿐이라 반복 발산이 아니라 **초기추정 의존 이중안정성**이 실체.

### 수정 — Modelica와 동일 처방

```python
w_wet = 0.5*(1 + tanh((T_dp - T_w)/dT_wet))     # dT_wet=0.2 K
b     = 1 + w_wet*(b_full - 1)                   # b→1 이면 건식 효율과 정확 일치
Q     = w_wet*Q_wet(엔탈피 포텐셜) + (1-w_wet)*Q_sens
Q_lat = w_wet*max(Q - Q_sens, 0)
```
히스테리시스·flip 카운팅 제거. T_w 물리 클램프와 적응 under-relax는 유지.
`fin_efficiency_wet(h_o, b=1)` = `fin_efficiency_schmidt(h_o)` 를 실측 확인(차 0)했으므로
극한에서 기존 건식 해와 정확히 이어진다.

### 검증

| 모드 | Q 전 → 후 | SH 전 → 후 | 미수렴 |
|---|---|---|---|
| row_parallel | 472.102 → 470.957 (−0.24 %) | 8.621 → 8.306 | 2 → **0** |
| serpentine_2 | 491.498 → 492.087 (+0.12 %) | 13.885 → 14.047 | 0 |
| serpentine_4 | 506.208 → 506.305 (+0.02 %) | 17.448 → 17.474 | 0 |
| single | — | — | 0 |

- **이중안정성 소멸**: "T_w 0.02 K 이내인데 w_wet 0.05 넘게 다른 쌍 = 0"
- `single` = `serpentine_4` 정확히 일치 (Nt=4 이므로 동일 배치 — 정합성 확인)
- **응축기 GT 불변** (552.39) — 건코일이라 `mode!="evap"` 경로, 영향 없음.
  따라서 응축기 관련 결론(D 누락·공기유량·반칸 규약)은 전부 유효.
- 남은 습/건 플래그 전환 3회는 인공물이 아니라 **실제 물리** — 행 경계(k=21, k=31)에서
  각 행이 다른 공기온도를 보아 T_w가 점프한다. `w_wet`은 0.14 → 0.51 → 0.68 → 0.86 으로 연속.

### GT 기준 변경 고지

증발기 GT가 최대 0.25 % 이동했다. 이 문서 상단의 Nseg 스윕 PY 값은 수정 **전** 기준이며,
증발기 OMC−PY 갭은 −2.1 % → 약 −1.8 %로 소폭 줄어든다(결론 불변 — 반칸 이산화 규약차).

## (구) 발견 2 — 습/건 플래그 진동 기록 (`a243da2` 시그니처)
