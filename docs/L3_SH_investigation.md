...
## 부록 8: 응축기 L3 재해석 — OMC h_o=302가 BC 불일치 오류, Python이 옳음

부록7의 cf_j 권고를 전면 정정. 사용자 지적("가정 없이 해석") 따라 h_o를
형상·유동에서 정직하게 검증한 결과, OMC h_o=302가 오류로 판명.

### 검증: j-factor·형상은 Python=OMC 완전 일치
같은 Re=1812에서:
  · OMC j_wang2000_plain = 0.01861
  · Python compute_j_factor(wang2000_plain) = 0.01894
  → j-factor 상관식 일치.
형상 A_c(최소유로): Python=OMC=0.00775 (동일 W,H,Pt,FPI로 계산). 일치.

### vendor 실제 h_o (CMM별) — OMC 하드코딩과 대조
              CMM=2.42   CMM=5.0    OMC 하드코딩
  증발기      176.4      340.1      176   ← CMM=2.42 일치 ✅
  응축기      179.2      345.5      302   ← 2.42도 5.0도 아님, ≈CMM4.4 ⚠️
증발기 OMC h_o=176은 L3 BC(2.42)와 정확히 일치(올바른 하드코딩).
응축기 OMC h_o=302는 L3 BC(2.42)의 물리값 179가 아닌, 더 높은 유량(≈4.4
CMM)의 값. j·형상 모두 일치하므로 유일한 설명 = OMC가 응축기 h_o를 다른
공기유량서 계산해 잘못 하드코딩.

### 결론: Python h_o=179가 물리적으로 옳음, OMC Q=633이 과대
- L3 BC CMM=2.42의 정직한 h_o = 179 (Python, j·형상서 계산).
- OMC h_o=302 = BC 불일치 하드코딩 오류.
- → OMC Q=633은 h_o 2배 과대로 부풀려진 값. Python Q=575가 물리적으로 정확.
- 그동안 'Python이 OMC보다 9% 낮다'를 Python 오류로 오해했으나, 실제로는
  OMC가 h_o 과대로 9% 높았던 것.

### 부록7 정정
cf_j=1.686 권고 철회. 이는 OMC의 BC 불일치 오류값(302)에 Python을 억지로
맞추는 것 = 오류를 정답으로 전제하는 순환논리. 적용하면 안 됨.
응축기 L3는 Python 현재값(h_o=179, Q=575)이 물리적으로 옳음. 보정 불필요.

### 남은 확인 (선택)
- OMC h_o=302의 출처: CondMBe.mo는 V_air_CMM=25.42(테스트값). h_o=302는
  CMM≈4.4서 나온 값 — 어느 스크립트/조건서 뽑았는지는 미확인(하드코딩이라
  소스에 근거 없음). 실무상 불필요.
- 실제 응축기 형상(N_rows=6, FPI=22, CondMBe.mo)을 Python에 반영 시 h_o=147,
  Q=565. 이것이 실제 형상 기준 물리값. 단 증발기형상(179)과 큰 차 아님.

### 최종 상태 (냉매측 12칸) — 정정판
압축기 L1✅L2✅L3✅ | EEV L1✅L2✅L3✅ |
응축기 L1✅L2✅L3✅(Python h_o 물리적 정확, OMC가 h_o 과대 오류) |
증발기 L1✅L2✅L3✅.
→ 12칸 전부 Python이 물리적으로 타당. OMC 대비 차이는 OMC 응축기 h_o
  하드코딩 오류(BC 불일치)에서 기인. Python 무보정이 옳음.

---

## 부록 9: h_o 하드코딩 전수 정리 — 실사용 계산전환 완료, 테스트는 fixture 유지

사용자 지적("컴포넌트에 하드코딩 없어야 맞다") 따라 OMC h_o 하드코딩 전수
조사·정리. 실사용 모델은 계산전환, 테스트 모델은 fixture로 유지 판단.

### h_o 하드코딩 위치 전수 (HPWDevap.mo, HPWDon.mo)
[실사용 — 계산 전환 완료]
  · Evap_On (L117): 176 → 계산 (FPI=20)
  · Evap_On_Dyn (L485): 176 → 계산 (L3 증발기, 사용)
  · Cond_On (L243): 302 → 계산 (FPI=22)
  · Cond_On_Dyn (L334): 302 → 계산 (L3 응축기, 사용)
[테스트 — fixture 유지]
  · Evap_On_Column (L37): 105.98 "입구기준 1회" — 셀진행 검증용
  · TestMarchSpan (HPWDon L467): 101.18 — march 로직 단위테스트
  · TestWetSeg (L557): 104.81 — wet 세그먼트 엔탈피포텐셜 테스트
  · TestWetDryMarch (L590): 104.81 — wet/dry 전환 테스트

### 계산 전환 방식 (실사용 4개)
h_o = j_wang2000_plain(Re_Dc,Nr,Dc,P_t,P_l,FPI,fin_t)·G_air·cp/Pr^(2/3).
  · final parameter → DAE 구조 불변, 콜드스타트 안정
  · V_air_CMM_ho로 BC 추종 (현 2.42, BC 바뀌면 h_o 따라감)
  · 형상 파라미터: 증발기 FPI=20, 응축기 FPI=22, A_o_face=0.0135744 공통
mu_air/Pr_air 켈빈 입력 준수(+273.15). W_humid/rho_humid_air 섭씨 유지.

### 결과 (실사용)
  증발기: h_o=180 Q=462.0 SH=5.87 (하드코딩 176·Python 176 일치, 무회귀)
  응축기: h_o=152 Q=577.9 x_out=0.251 (Python 147~179·565~575 일치)
→ 응축기 h_o 오류(302) 근본 수정, 증발기 하드코딩 제거하며 값 유지.

### 테스트 모델 fixture 유지 판단 (계산 전환 안 함)
- 실사용 원칙 '물리모델은 계산'은 사이클/폐루프서 BC 다양한 대상용.
- 테스트 모델은 '알려진 입력→알려진 출력' 알고리즘 검증 목적. h_o는
  고정 fixture 역할(예: Evap_On_Column 105.98='입구기준 1회' 특정조건).
- 계산 전환 시 테스트 입력이 BC 따라 변동 → march/wet-dry 검증 의도 흐려짐.
- → 테스트 모델 h_o 하드코딩은 의도적 fixture로 유지가 타당.

### 최종 (실사용 냉매측 12칸 + 하드코딩 정리)
압축기·EEV·응축기·증발기 L1L2L3 전부 물리적 타당.
실사용 4개 L3 HX 모델 h_o 하드코딩 전부 제거 → 형상·유동서 산출.
OMC-Python 응축기 L3 정합(하드코딩 오류 근본 해결).
테스트 모델 h_o는 fixture로 유지(검증 재현성).
